...
void myText(Double_t x,Double_t y,Color_t color, const char *text, Double_t tsize = 0.03) {
...
void myBoxText(Double_t x, Double_t y,Double_t boxsize,Int_t mcolor, Float_t alpha, const char *text, Double_t lstyle, Double_t lcolor, Double_t mstyle, bool stat,Double_t tsize=0.03,int FillStyle = 1001) 
{
  ...
  mbox->SetFillColorAlpha(mcolor, alpha);
  mbox->SetFillStyle(FillStyle);
  mbox->Draw();

  Double_t y_new=(y1+y2)/2.;
    Double_t x_new=(x1+x2)/2.;  
    TMarker *marker = new TMarker(x_new,y_new,8);
  marker->SetMarkerColor(lcolor);  marker->SetNDC();
...
void myMarkerText(Double_t x,Double_t y,Int_t color,Int_t mstyle, const char *text,Float_t msize, Float_t tsize) 
{
  //Double_t tsize=0.06;
...
TH1F* hotTH1F(const char* name, const char* title, int nbins, float xmin, float xmax, const char* x, const char* y, int color, float msize, int MarkerStyle, int LineStyle, bool sumw2){

  TH1F* g1 = new TH1F(name, title, nbins, xmin, xmax);
  ...
  //g1->SetFillColorAlpha(color,0.35);
...
TH1F* hotTH1F(const char* name, const char* title, int nbins, Float_t *xbins, const char* x, const char* y, int color, float msize, int MarkerStyle, int LineStyle, bool sumw2){

  TH1F* g1 = new TH1F(name, title, nbins, xbins);
...
TMultiGraph* hotTMultiGraph(const char* title, const char* x, const char* y, Double_t xl, Double_t xh, Double_t yl, Double_t yh){
  ...
  g1->GetXaxis()->SetNdivisions(509,kTRUE);
  g1->GetHistogram()->SetTitle(title);
  g1->GetXaxis()->SetTitle(x);
  g1->GetYaxis()->SetTitle(y);

  g1->GetHistogram()->GetXaxis()->SetLimits(xl,xh);
  g1->GetHistogram()->GetXaxis()->SetRangeUser(xl,xh);
  g1->GetHistogram()->GetYaxis()->SetLimits(yl,yh);
  g1->GetHistogram()->GetYaxis()->SetRangeUser(yl,yh);
...
TGraphErrors* hotTGraphErrors(const char* name, const char* title, int color, double msize, int MarkerStyle, int LineStyle, int FillStyle, float FillAlpha){
  ...
  g1->SetMarkerSize(msize);
  g1->SetLineStyle(LineStyle);
  g1->SetFillStyle(FillStyle);
  g1->SetFillColorAlpha(color,FillAlpha);
  return g1;
...
/*TGraphAsymmErrors* hotTGraphErrors(const char* name, const char* title, int color, int MarkerStyle, int LineStyle, int FillStyle, float FillAlpha){
  ...
  g1->SetLineStyle(LineStyle);
  g1->SetFillStyle(FillStyle);
  g1->SetFillColorAlpha(FillAlpha);
  g1->SetFillColor(color);
}*/
